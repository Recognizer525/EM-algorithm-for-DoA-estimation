...
Предположим, имеется линейная равномерная антенная решетка, состоящая из $L$ сенсоров, которая принимает узкополосные сигналы, направленные из $K$  источников, причем $K < L$. Этим источникам соответствуют угловые координаты (DoA) $\theta = [\theta_1, \ldots, \theta_{K}]^T$, практически не изменяющиеся во времени. По итогам измерений было получено $T$ снимков полученного сигнала, причем ввиду стохастических технических сбоев, связанных с сенсорами, большая часть таких снимков содержит помимо надежных данных ненадежные, которые в рамках данной задачи рассматриваются как пропуски.  Матрица принятых сигналов (наблюдений) $X$ является результатом следующей модели наблюдений:
...
\end{enumerate}
На практике, не требуется напрямую находить ковариации для распределений $q(\Xi_t), t=1,...,T$, достаточно лишь найти условные вторые моменты $\Mexp_{q(\Xi)}[XX^*]$, $\Mexp_{q(\Xi)}[XS^*]$, $\Mexp_{q(\Xi)}[SS^*]$. Ввиду того, что наблюдения $X_t, t = 1,\ldots, T$ являются независимыми и одинаково распределенными, мы можем найти их путем усреднения соответствующих условных вторых моментов по всем имеющимся наблюдениям:
\begin{equation*}
\Mexp_{q(\Xi)}[XX^*] = \frac{1}{T}\sum_{t=1}^T \Mexp[X_t X_t^* \mid \mathcal{I}_t],
\end{equation*}
\begin{equation*}
\Mexp_{q(\Xi)}[S S^*] =  \frac{1}{T} \sum_{t=1}^T \Mexp[S_t S_t^* \mid \mathcal{I}_t],
\end{equation*}
\begin{equation*}
\Mexp_{q(\Xi)}[X S^*] =  \frac{1}{T} \sum_{t=1}^T \Mexp[X_t S_t^* \mid \mathcal{I}_t].
\end{equation*}
Рассмотрим случай, когда наблюдение $X_t$ является полностью доступным, т.е. $m_t = \varnothing$. Тогда, апостериорное распределение $\Prob(X_{t,m_t},S_t \mid \mathcal{I}_t)$ сводится к распределению $\Prob(S_t \mid \mathcal{I}_t)$, а условные моменты $\Mexp[X_t X_t^* \mid \mathcal{I}_t]$, $\Mexp[X_t S_t^* \mid \mathcal{I}_t]$, $\Mexp[S_t S_t^* \mid \mathcal{I}_t]$ вычисляются по следующей схеме. Находим параметры апостериорного распределения $\Prob(S_t \mid \mathcal{I}_t), t = 1,\ldots, T$:
\begin{equation*}
\left\{ \begin{aligned} 
\Mexp[S_t \mid \mathcal{I}_t] &= \bGamma^*\bA^* \bR^{-1}x_t, \\
\cov(S_t \mid \mathcal{I}_t) &= \bGamma - \bGamma^*\bA^*  \bR^{-1}\bA\bGamma.
\end{aligned} \right.
\end{equation*}
И вычисляем условные моменты следующим образом:
\begin{equation*}
\Mexp [X_t X_t^* \mid \mathcal{I}_t] = \Mexp[X_t \mid \mathcal{I}_t] \cdot \Mexp[X_t^* \mid \mathcal{I}_t] = x_t x_t^*,
\end{equation*}
\begin{equation*}
\Mexp [X_t S_t^* \mid \mathcal{I}_t] = \Mexp[X_t \mid \mathcal{I}_t] \cdot \Mexp[S_t^* \mid \mathcal{I}_t] = x_t \cdot \Mexp[S_t^* \mid \mathcal{I}_t] = x_t \cdot x_t^* \cdot \bR^{-1} \bA \bGamma,
\end{equation*}
\begin{equation*}
\Mexp [S_t S_t^* \mid \mathcal{I}_t] = \Mexp[S_t \mid \mathcal{I}_t] \cdot \Mexp[S_t^* \mid \mathcal{I}_t] + \cov(S_t \mid\mathcal{I}_t). 
\end{equation*} \\
...
Параметры апостериорного распределения $\Prob(X_{t, m_t} \mid \mathcal{I}_t), t=1,\ldots,T$ на итерации $\tau$ можно найти следующим образом:
\begin{equation*}
\left\{ \begin{aligned} 
\Mexp[X_{t, m_t} \mid \mathcal{I}_t] &= \bR_{m_t, o_t}\left(\bR_{o_t, o_t}\right)^{-1}\cdot x_{t, o_t}, \\
\cov(X_{t, m_t}\mid \mathcal{I}_t) &= \bR_{m_t, m_t}-\bR_{m_t, o_t}\left(\bR_{o_t, o_t}\right)^{-1}\bR_{o_t, m_t},
\end{aligned} \right.
\end{equation*}
...
Для каждого наблюдения $X_t$, содержащего пропуски, определим условные первый и второй начальный момент $\Mexp[X_t X_t^* \mid \mathcal{I}_t]$:
\begin{equation*}
\begin{gathered}
\Mexp[X_t \mid \mathcal{I}_t] = \Mexp \bigg[\begin{pmatrix} X_{t,o_t} \\ X_{t,m_t}\end{pmatrix}\Big| \mathcal{I}_t\bigg] 
= \begin{pmatrix} x_{t,o_t} \\ \Mexp[X_{t,m_t} \mid \mathcal{I}_t]\end{pmatrix},\\
\Mexp[X_t X_t^* \mid \mathcal{I}_t] = \Mexp [X_t \mid \mathcal{I}_t] \cdot \Mexp [X_t^* \mid \mathcal{I}_t] + \cov (X_t \mid \mathcal{I}_t) \\
= \Mexp [X_t \mid \mathcal{I}_t] \cdot \Mexp [X_t^* \mid \mathcal{I}_t] +
\begin{pmatrix}
...
Параметры апостериорного распределения $\Prob(S_t \mid \mathcal{I}_t), t = 1,\ldots, T$ можно найти следующим образом:
\begin{equation*}
\left\{ \begin{aligned} 
\Mexp[S_t \mid \mathcal{I}_t] &= \bGamma^*\bA^* \bR^{-1}\Mexp [X_t \mid \mathcal{I}_t], \\
\cov(S_t \mid \mathcal{I}_t) &= \bGamma - \bGamma^*\bA^*  \bR^{-1}\bA\bGamma + \bGamma^*\bA^* \bR^{-1}\cov(X_t \mid \mathcal{I}_t)\bR^{-1} \bA\bGamma,
\end{aligned} \right.
\end{equation*}
...
Оценим $\Mexp [S_t S_t^* \mid \mathcal{I}_t]$:
\begin{equation*}
\Mexp [S_t S_t^* \mid \mathcal{I}_t] = \Mexp[S_t \mid \mathcal{I}_t] \cdot \Mexp[S_t^* \mid \mathcal{I}_t] + \cov(S_t \mid \mathcal{I}_t),
\end{equation*}
Оценим $\Mexp [X_t S_t^* \mid \mathcal{I}_t]$:
\begin{equation*}
\begin{gathered}
\Mexp [X_t S_t^* \mid \mathcal{I}_t] = \Mexp[X_t X_t^* \mid \mathcal{I}_t]\bR^{-1}\bA\bGamma.
\end{gathered}
\end{equation*}
\subsection{М-шаг}
...
\begin{equation*}
\begin{gathered}
L = 25, K = 1, T = 12, \text{5 сенсоров неисправны, 50\% пропусков для каждого из них}, \\ \bGamma = 0.5 \cdot \text{E}_{K}, \bLambda = 8.1 \cdot \text{E}_{L}, \theta = [0.7] \approx [40.107^{\circ}].
\end{gathered}
\end{equation*}
\begin{figure}[h]
    \centering
...
\begin{equation*}
\begin{gathered}
L = 25, K = 1, T = 11, \text{8 сенсоров неисправны, 50\% пропусков для каждого из них},\\  \bGamma = 0.5 \cdot \text{E}_{K}, \bLambda = 6.1 \cdot \text{E}_{L}, \theta = [0.7] \approx [40.107^{\circ}].
\end{gathered}
\end{equation*}
\begin{figure}[h]
    \centering

...
\begin{abstract}
В~работе построен ЕМ-алгоритм для оценивания угловых координат неподвижных объектов при наличии пропусков в наблюдениях.
\end{abstract}
...
Предположим, имеется линейная равномерная антенная решетка, состоящая из $\mathrm{L}$ сенсоров, которая принимает узкополосные сигналы, направленные из $\mathrm{K}$  источников, причем $\mathrm{K} < \mathrm{L}$. Этим источникам соответствуют угловые координаты (DoA) $\theta = [\theta_1, \ldots, \theta_{\mathrm{K}}]^T$, практически не изменяющиеся во времени. По итогам измерений было получено $\mathrm{T}$ снимков полученного сигнала, причем ввиду стохастических технических сбоев, связанных с сенсорами, большая часть таких снимков содержит помимо надежных данных ненадежные, которые в рамках данной задачи рассматриваются как пропуски.  Матрица принятых сигналов (наблюдений) $X$ является результатом следующей модели наблюдений:
...
\end{bmatrix},
\end{equation*}
где $i$ -- мнимая единица, $d$ -- расстояние между сенсорами, $\lambda$ -- длина волны, $\theta_k$ -- угловая координата $k$-го источника, $k = 1, \ldots, \mathrm{K}$. Через $x = [x_1,\ldots, x_\mathrm{T}]$ будем обозначать реализацию матрицы $X=[X_1,\ldots, X_\mathrm{T}]$, а через $x_t$ будем обозначать реализацию наблюдения $X_t$. Для каждого момента времени $t=1,\ldots,\mathrm{T}$ определено непустое множество индексов, соответстветствующих доступным (для наблюдения) компонентам вектора $X_t$: $o_t \subseteq \{1, \ldots, \mathrm{L}\}, \quad 1 \leq |o_t| \leq \mathrm{L}$ и множество индексов, соответствующих пропускам в векторе $X_t$: $m_t \subset \{1, \ldots, \mathrm{L}\}$, причем $o_t \cup m_t = \{1,\ldots,\mathrm{L}\}, o_t \cap m_t = \varnothing,  \forall t \in \{1,\ldots,\mathrm{T}\}$. Доступная и недоступная части вектора $X_t$ обозначаются через $X_{t,o_t} = ((X_t)_i)_{i \in o_t} \in \Complex{С}^{|o_t|}$ и $X_{t,m_t} = ((X_t)_i)_{i \in m_t} \in \Complex{С}^{|m_t|}$ соответственно. Таким образом, будем считать, что набор наблюдений $X$ состоит из доступной части $X_o = \{X_{t, o_t}\}_{t=1}^\mathrm{T}$ и недоступной: $X_m = \{X_{t, m_t}\}_{t=1}^\mathrm{T}$,
Cигналы, испускаемые источниками, шумы на сенсорах и наблюдения предполагаются стохастическими: $S_t \sim \NormComplex(\mathbf{O}_{\mathrm{K} \times 1},\mathbf{\Gamma})$, $N_t \sim \NormComplex(\mathbf{O}_{\mathrm{L} \times 1}, \mathbf{\Lambda})$, $ X_t \sim \NormComplex(\mathbf{O}_{\mathrm{L} \times 1}, \mathbf{A}(\theta)\mathbf{\Gamma}\mathbf{A}(\theta)^* + \mathbf{\Lambda})$, $t=1,\ldots,\mathrm{T}$, где $\NormComplex$ соответствует комплексному нормальному распределению с нулевой псевдоковариацией. Матрица $\mathbf{\Lambda}$ предполагается диагональной, т.е. шумы не коррелированы между собой, также в любой момент времени $S_t \perp N_t$. Требуется найти оптимальные оценки параметров $\Upsilon = (\theta, \mathbf{\Gamma})$.
...
\end{itemize}
\subsection{E-шаг}
Из построения модели наблюдений следует независимость и одинаковая распределенность наблюдений $X_t$, $t=1,\ldots,\mathrm{T}$. Требуется найти математическое ожидание полного правдоподобия с учетом  текущей оценки параметров и апостериорного совместного распределения пропущенных значений в наблюдениях $X_m$ и сигналов $S$ 
\begin{equation*}
 \Expect_{q(\Xi)}[\log \Prob(X, S)]= \Expect_{(X_m,S) \mid X_o=x_o, \Upsilon^{(\tau-1)}}[\log \Prob(X, S)].
\end{equation*}
Преобразуем выражение, учитывая тот факт, что наблюдения являются независимыми и одинаково распределенными, и обозначив через $\mathcal{I}$ доступную информацию $X_o=x_o, \Upsilon^{(\tau-1)}$  на итерации $\tau$  и через $q(\Xi_t)$ распределение $\Prob(X_{t,m_t},S_t \mid X_{t,o_t}=x_{t,o_t}, \Upsilon^{(\tau-1)})$:
\begin{equation*}
\begin{gathered}
 \Expect_{q(\Xi)}[\log \Prob(X, S)] = \\
 \Expect_{q(\Xi)}[\log \Prob(X\mid S) + \log \Prob(S)] = \\
\Big[\sum_{t=1}^\mathrm{T}  \Expect_{q(\Xi_t)} \log [\Prob(X_t \mid S_t)] + \sum_{t=1}^\mathrm{T} \Expect_{q(\Xi_t)}[\log \Prob(S_t)]\Big] = \\
...
\end{enumerate}
На практике, не требуется напрямую находить ковариации для распределений $q(\Xi_t), t=1,...,\mathrm{T}$, достаточно лишь найти условные вторые моменты $\Expect_{q(\Xi)}[XX^*]$, $\Expect_{q(\Xi)}[XS^*]$, $\Expect_{q(\Xi)}[SS^*]$. Ввиду того, что наблюдения $X_t, t = 1,\ldots, \mathrm{T}$ являются независимыми и одинаково распределенными, мы можем найти их путем усреднения соответствующих условных вторых моментов по всем имеющимся наблюдениям:
\begin{equation*}
\Expect_{q(\Xi)}[XX^*] = \frac{1}{\mathrm{T}}\sum_{t=1}^\mathrm{T} \Expect[X_t X_t^* \mid \mathcal{I}_t],
\end{equation*}
\begin{equation*}
\Expect_{q(\Xi)}[S S^*] =  \frac{1}{\mathrm{T}} \sum_{t=1}^\mathrm{T} \Expect[S_t S_t^* \mid \mathcal{I}_t],
\end{equation*}
\begin{equation*}
\Expect_{q(\Xi)}[X S^*] =  \frac{1}{\mathrm{T}} \sum_{t=1}^\mathrm{T} \Expect[X_t S_t^* \mid \mathcal{I}_t].
\end{equation*}
Рассмотрим случай, когда наблюдение $X_t$ является полностью доступным, т.е. $m_t = \varnothing$. Тогда, апостериорное распределение $\Prob(X_{t,m_t},S_t \mid \mathcal{I}_t)$ сводится к распределению $\Prob(S_t \mid \mathcal{I}_t)$, а условные моменты $\Expect[X_t X_t^* \mid \mathcal{I}_t]$, $\Expect[X_t S_t^* \mid \mathcal{I}_t]$, $\Expect[S_t S_t^* \mid \mathcal{I}_t]$ вычисляются по следующей схеме. Находим параметры апостериорного распределения $\Prob(S_t \mid \mathcal{I}_t), t = 1,\ldots, \mathrm{T}$:
\begin{equation*}
\left\{ \begin{aligned} 
\Expect[S_t \mid \mathcal{I}_t] &= \mathbf{\Gamma}^*\mathbf{A}^* \hat{\mathbf{R}}^{-1}x_t, \\
...
\begin{equation*}
\check{\mathbf{\Gamma}}^{(0)}=\mathcal{D}\big[\check{\mathbf{A}}(\theta^{(0)})^{+} (\hat{\mathbf{R}}^{(0)}-\mathbf{\Lambda})(\check{\mathbf{A}}(\theta^{(0)})^{+})^*\big],
\end{equation*}
где $\mathcal{D}\big[ \cdot \big]$ -- диагональное приближение матрицы, $(\cdot)^{+}$ -- псевдообратная матрица. Такой подход не гарантирует, что все элементы матрицы на главной диагонали будут неотрицательными, поэтому используем следующую коректировку: $\check{\mathbf{\Gamma}}_{ll}^{(0)} = \max(\check{\mathbf{\Gamma}}_{ll}^{(0)}, \varepsilon)$, $l=1,\ldots,\mathrm{L}$. Переходим к ненормированной мощности:
\begin{equation*}
\mathbf{\Gamma}_{ll}^{(0)}=\frac{\check{\mathbf{\Gamma}}_{ll}^{(0)}}{||a(\theta^{(0)}_k)||_l}, l=1,\ldots,\mathrm{L}.
\end{equation*}
...
\begin{equation*}
\mathcal{G}(u \mid u^{(\tau-1)}) = \mathcal{Q}_1(u^{(\tau-1)} \mid u^{(\tau-1)}) + \nabla \mathcal{Q}_1(u^{(\tau-1)} \mid u^{(\tau-1)})^T (u-u^{(\tau-1)}) + \frac{1}{2} (u-u^{(\tau-1)})^T \mathbf{H} (u-u^{(\tau-1)}),
\end{equation*}
где
\begin{equation*}
{\small
...
\dots&\dots&\ddots&\vdots\\
0& 0&\dots& \max(|\nabla_\mathrm{K} \mathcal{Q}_1(u^{(\tau-1)} \mid u^{(\tau-1)})|,\varepsilon)\\
\end{bmatrix},
}
\end{equation*}
причем $\nabla_1 \mathcal{Q}_1(u^{(\tau-1)}\mid u^{(\tau-1)})$ -- $i$-я компонента градиента $\mathcal{Q}_1(u^{(\tau-1)}\mid u^{(\tau-1)})$. Диагональная аппроксимация $\mathbf{H}$ предотвращает слишком маленькие или слишком большие шаги, сохраняя численную стабильность.
...
\begin{equation*}
\nabla \mathcal{Q}_1(u^{(\tau-1)}\mid u^{(\tau-1)})^T \rho^{(\tau)} = - \nabla \mathcal{Q}_1(u^{(\tau-1)}\mid u^{(\tau-1)})^T \mathbf{H}^{-1}  \nabla \mathcal{Q}_1(u^{(\tau-1)}\mid u^{(\tau-1)}),
\end{equation*} 

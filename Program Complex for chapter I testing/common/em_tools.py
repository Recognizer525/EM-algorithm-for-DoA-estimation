...
from . import estim_angles_pgd as eap
...
def EM(angles: np.ndarray,
    ...
    is_sur_opt: bool
        Использовать ли ускоренную оптимизацию (задействует суррогат оптимизируемой функции).
    show_lkhd: bool
    ...
    while EM_Iteration <= max_iter:
        ...
        Sigma_XX_arr = sn.cov_correcter(Sigma_XX_arr, 0.01 * reg_coef)
        ...
        if is_sur_opt:
            ...
            new_angles = eap.find_angles(Sigma_XS, angles, Sigma_SS, Q_inv)
